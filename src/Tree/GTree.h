...
public:
    ...
    SharedPointer< Tree<T> > remove(const T& value)     //删除操作将目标结点所代表的子树移除
    {
        GTree<T>* ret = NULL;
    ...
    GTreeNode<T>* find(const T& value)const //查找操作最终返回目标数据地址
    {
        return find(root(), value);
    }
    GTreeNode<T>* find(TreeNode<T>* node)   //查找操作最终返回目标结点地址
    {
